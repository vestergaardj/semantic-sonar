...
export default function BrowseClient() {
  ...
  // Sort alphabetically, then filter by search term (supports * wildcard)
  const filteredWorkspaces = (() => {
    ...
    if (!q) return sorted;
    // Convert wildcard pattern to regex: * → .*, escape the rest
  ...
  return (
    <div className="space-y-6">
      ...
      <section>
        ...
        {/* Selected workspace pill (shown when collapsed) */}
        {wsCollapsed && selectedWs && (
          <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-brand-200 bg-brand-50 px-3 py-1 text-sm dark:border-brand-800 dark:bg-brand-950">
            <span className="font-medium text-brand-700 dark:text-brand-300">{selectedWs.name}</span>
            <button
          ...
          <>
            ...
            {workspaces.length > 0 && (
              <input
                ...
                value={wsSearch}
                onChange={(e) => setWsSearch(e.target.value)}                aria-label="Search workspaces"                className="mb-3 w-full max-w-md rounded-md border border-gray-300 bg-white px-3 py-1.5 text-sm text-gray-900 placeholder-gray-400 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 dark:placeholder-gray-500"
              />
            )}
            ...
            {loadingWs ? (
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                No workspaces match &ldquo;{wsSearch}&rdquo;
              </p>
            ) : (

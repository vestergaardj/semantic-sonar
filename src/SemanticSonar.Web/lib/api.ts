...
export const resultsApi = {
  ...
  tagGroups: () => request<TagGroupSummary[]>('/tag-groups'),

  performanceBudgets: () => request<PerformanceBudgetResult[]>('/performance-budgets'),
};

// ── Tags ────────────────────────────────────────────────────────────────────

export const tagsApi = {
  list: () => request<TagDefinition[]>('/tag-definitions'),

  create: (name: string) =>
    request<TagDefinition>('/tag-definitions', {
      method: 'POST',
      body: JSON.stringify({ name }),
    }),

  update: (id: string, name: string) =>
    request<TagDefinition>(`/tag-definitions/${id}`, {
      method: 'PUT',
      body: JSON.stringify({ name }),
    }),

  delete: (id: string, force = false) =>
    request<void>(`/tag-definitions/${id}${force ? '?force=true' : ''}`, {
      method: 'DELETE',
    }),

  usage: (id: string) =>
    request<{ tagId: string; tagName: string; usageCount: number }>(
      `/tag-definitions/${id}/usage`,
    ),

  seed: () =>
    request<{ seeded: number }>('/tag-definitions/seed', { method: 'POST' }),
};

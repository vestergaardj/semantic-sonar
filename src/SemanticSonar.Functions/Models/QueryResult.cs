...
/// <summary>Aggregated health summary grouped by tag.</summary>
public class TagGroupSummary
{
    public string Tag { get; set; } = "";
    public int TotalModels { get; set; }
    public int ActiveModels { get; set; }
    public int FailingModels { get; set; }
    public double AvgHealthScore { get; set; }
    public string AvgGrade { get; set; } = "";
    public double AvgUptimePercent { get; set; }
}

// ── Performance Budgets ──────────────────────────────────────────────────────

/// <summary>
/// Per-model performance budget status based on a rolling latency baseline.
/// The rolling window is split: the older half is the baseline, the newer half
/// is compared against it using configurable warn/critical multipliers.
/// </summary>
public class PerformanceBudgetResult
{
    ...
    /// <summary>
    /// Budget status: "ok", "warn", "critical", or "insufficient-data".
    /// </summary>
    public string Status { get; init; } = "insufficient-data";

    /// <summary>P50 latency of the baseline (older) half of the sample window, in ms.</summary>
    public long? BaselineP50Ms { get; init; }

    /// <summary>P50 latency of the recent (newer) half of the sample window, in ms.</summary>
    public long? CurrentP50Ms { get; init; }

    /// <summary>Most recent single execution latency, in ms.</summary>
    public long? LatestLatencyMs { get; init; }

    /// <summary>Threshold at which WARN fires (baseline × warnMultiplier), in ms.</summary>
    public long? WarnThresholdMs { get; init; }

    /// <summary>Threshold at which CRITICAL fires (baseline × criticalMultiplier), in ms.</summary>
    public long? CriticalThresholdMs { get; init; }

    public int SampleSize { get; init; }
    public double WarnMultiplier { get; init; }
    public double CriticalMultiplier { get; init; }

    /// <summary>
    /// Last N latency values in chronological order (oldest first), for sparkline rendering.
    /// Only successful executions are included.
    /// </summary>
    public List<long> RecentLatencies { get; init; } = [];
}

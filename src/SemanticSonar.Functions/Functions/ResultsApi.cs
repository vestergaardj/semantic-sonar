...
public class ResultsApi
{
    ...
    [Function("GetPerformanceBudgets")]
    public async Task<IActionResult> GetPerformanceBudgets(
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "performance-budgets")] HttpRequest req)
    {
        var authResult = AuthHelper.EnforceAuth(req);
        if (authResult != null) return authResult;

        var budgets = await _cosmos.GetPerformanceBudgetsAsync();

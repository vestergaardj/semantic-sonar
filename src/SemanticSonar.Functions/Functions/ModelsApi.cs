...
public class ModelsApi
{
    ...
    private static string? ValidateModel(SemanticModelConfig m)
    {
        ...
        if (m.Tags is not null && m.Tags.Count > 10)
            return "Maximum 10 tags per model.";
        if (m.Tags is not null && m.Tags.Any(t => t.Length > 30))
